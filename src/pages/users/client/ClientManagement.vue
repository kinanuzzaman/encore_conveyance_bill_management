<template>
  ...
  <main>
    ...
    <section class="mx-4 py-6 flex justify-between">
      <div class="text-2xl font-semibold">Client Management</div>

      <div class="flex gap-5">
        <q-btn color="blue" @click="$router.push('client/pending')" class="rounded-lg" unelevated
          label="Pending Client" />
        <q-btn color="orange" v-if="authStore.canAccess('user_create')" @click="confirm = true" class="rounded-lg"
          unelevated icon="add" label="Add Client" />
      </div>
    </section>
    <!-- Table -->
    ...
    <q-dialog v-model="confirm" class="" persistent>
      <q-card class="my-card p-10" style="width: 1020px; max-width: 80vw">
        ...
        <span class="text-xl py-5 text-center">Invite new client</span>
  ...
  setup() {
    ...
    async function toggleUserStatus(user) {
      try {
        ...
        tableRef.value.requestServerInteraction();
        $q.notify({
          message: "Client User status updated",
          color: "positive",
          position: "top",
        });
      ...
      $q.dialog({
        ...
        apiSerivce.delete(`/users/${id}`).then(() => {
          $q.notify({
            ...
            position: "top",
            icon: 'cloud_done',
            message: 'Client user record deleted successfully'
          })
          tableRef.value.requestServerInteraction();
        }).catch(err => {
          $q.notify({
